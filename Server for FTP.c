		...
		if(server_receive(server_sock, buf, BUFFER, NULL) > 0)// returns a pointer to the buffer.
		{
			...
			switch(hdr->pkt_type)
			{
				...
				//	puts("RTT2 sent");
				 ...
				 else if(((struct rtt *) (buf + HDR_SIZE))->type == RTT5)
				 {
...
//					strcpy(recvdata->data,((struct filetrans *)(buf+HDR_SIZE))->data);
					
					if(hdr->dest_id==router.my_id)
					{
						printf("\n Packet with sequence number %lu received!\n Data received: %s\n",hdr->seq,((void *)(buf+HDR_SIZE)));
					}
					seq=hdr->seq+1;
					for(j=0;j<n;j++)
					{
					...
					create_pkt(send_buf, router.my_id, FILE , PKTSIZE,(void *)"ACK",seq,hdr->router_id);
					client_send(router.ips[r], router.ports[r], send_buf, HDR_SIZE + PKTSIZE);
					printf("\n ACK with sequence number %lu sent!\n",seq);
					free(send_buf);
//					else
//					{
//						int i;
...
//						}
//						printf("\n Next hop is %lu\n",next_hop);
//						void *send_buf = malloc(HDR_SIZE + sizeof(struct filetrans));
//						create_pkt(send_buf, router.my_id, FILE, sizeof(struct filetrans), (void *)&recvdata,0,hdr->dest_id);
//						client_send(router.ips[next_hop], router.ftr_sendports[next_hop], send_buf, HDR_SIZE + sizeof(struct filetrans));
//						free(send_buf);
//					}
